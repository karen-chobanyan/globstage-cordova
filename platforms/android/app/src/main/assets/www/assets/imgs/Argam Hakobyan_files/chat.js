...
var im_chat = {
	...
	opens:function(uid,s,on){
		if($('#im_chats'+uid).is('.im_chats')){
			...
			$.post('/index.php?go=im_chat&act=history', {for_user_id: uid}, function(d){
				$('body').append('<div id="sc'+uid+'"><script type="text/javascript">$(document).ready(function(){chat_interval_im = setInterval("im_chat.update('+uid+')", 10000);});</script></div><div class="main-chats"><div id="im_chats'+uid+'" class="im_chats "><div class="im_chatclose fl_r" style="margin-top: 7px;" onclick="im_chat.closes(\''+uid+'\');"><i class="fa fa-times" aria-hidden="true"></i></div><div id="im_chatName'+uid+'" class="im_chatname fl_l" onclick="im_chat.hideShow(\''+uid+'\');">'+s+'</div> <div class="'+on+' fl_l" style="margin-top:12px"></div><div class="clear"></div><div id="imViewMsg'+uid+'">'+d+'</div></div></div>');
	...
	send: function(for_user_id, my_name, my_ava, msg){
		var msg_text = $('#msg_text'+for_user_id).val();
    var attach_files = $('#vaLattach_files-'+for_user_id).val();  
		if(msg_text != 0 || attach_files != 0){
			$.post('/index.php?go=im_chat&act=send', {for_user_id: for_user_id, my_name: my_name, my_ava: my_ava, msg: msg_text, attach_files: attach_files}, function(data){
				if(data == 'err_privacy')
          ...
          $('#vaLattach_files-'+for_user_id).val('');          
          $('#attach_files-'+for_user_id).html('');
          $('#attach_files-'+for_user_id).hide();
				}
			});
		} else{
