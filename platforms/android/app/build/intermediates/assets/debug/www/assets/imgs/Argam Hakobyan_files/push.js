...
$(document).ready(function(){
	setInterval(function(){
		$.post('/index.php?go=updates', function(d){
			row = d.split('|');
			if(d && row[1]){
				if(row[0] == 1) uTitle = new_com_wall;
				else if(row[0] == 2) uTitle = new_com_photos;
				else if(row[0] == 3) uTitle = new_com_videos;
				else if(row[0] == 4) uTitle = new_com_note;
				else if(row[0] == 5) uTitle = new_com_com;
				else if(row[0] == 6) uTitle = new_com_themes;
				else if(row[0] == 7) uTitle = new_gifts;
				else if(row[0] == 8) uTitle = new_msg;
				else if(row[0] == 9) uTitle = new_evaluation;
				else if(row[0] == 10) uTitle = your_record_was_like;
				else if(row[0] == 11) uTitle = new_application;
				else if(row[0] == 12) uTitle = application_accepted;
				else if(row[0] == 13) uTitle = subscriptions;
				else uTitle = notifi;
				if(row[0] == 8){
					sli = row[6].split('/');
