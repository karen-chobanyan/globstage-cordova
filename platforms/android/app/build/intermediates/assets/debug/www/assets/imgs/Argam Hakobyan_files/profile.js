...
var ads = {
  ...
  send: function(){
    ...
    $.post('/index.php?go=ads&act=add_ads', {link_photos: link_photos, link_site: link_site, title: title, description: description, category: category, transitions: transitions, cost_num: cost_num}, function(d){
        if(d == '1'){
            var result = payment_process_completed;    
        }
        else if(d == '2'){
            var result = 'all_fields_are_required';    
        }
        else if(d == '3'){
            var result = Insufficient_amount_of_funds;    
        }
			butloading('sending', '56', 'enabled', order);
			$('#result').show();
			$('#result').html(result);
		});
	},
	...
	delete_ads: function(id){
		$('#ads_'+id).html('<div style="padding:10px;color:#666;"><center>'+funds_purchase_were_returned+'</center></div><div class="clear"></div>');
	...
	edit_save: function(id){
		...
		if(d == '1'){
            var result = ad_changes_successfully_saved;  
        }
			$('#result_'+id).show();
...
function set_my_html(){
	myhtml.title('1', lang_select_the_language_used_on_the_site, 'langTitle', 1);
...
box: function(user_id, c){
	...
	delet: function(gid){
		$('#gift_'+gid).html('<div class="color77777" style="margin-bottom:5px">'+gift_deleted+'</div>');
		updateNum('#num');
...
var pins = {
  ...
  fast_comm_del_m: function(id){
    $('#comment_'+id).html(lang_wall_del_com_ok);
    $.post('/index.php?go=pins&act=del_comm', {id: id}, function(d){
      $('#comment_'+id).remove();
    });
    Box.Close('box_comment_'+id);
  },
  
  
	del_comm: function(id){
      Box.Show('comment_'+id, 400, lang_title_del_photo, '<div style="padding:15px;" id="text_del_quest">'+lang_del_note+'</div>', lang_box_canсel, lang_box_yes, 'pins.fast_comm_del_m('+id+'); return false');	
	},
...
var confirmation = {
	confirm_yes: function(user_id, stars_id){
		$.post('/index.php?go=confirmation&act=confirm_yes', {user_id: user_id, stars_id: stars_id}, function(d){
			if(d == '1'){
				Box.Info('msg_info', successfully, saved_successfully, 300);
			}
			else if(d == '2'){
				Box.Info('msg_info', error, unknown_error, 300);
			}
			setInterval(function(){
				$("#user_"+user_id).remove();
				$("#confirm_"+user_id).html('<center style="color:#808080;">'+operation_completed+'</center>');
			}, 1000);
	...
	confirm_send: function(uid){
		$.post('/index.php?go=confirmation&act=confirm_send', {user_id: uid}, function(d){
			if(d == '1'){
				Box.Info('msg_info', successfully, application_sent, 300);
			}
			else if(d == '2'){
				Box.Info('msg_info', error, unknown_error, 300);
			}
	...
	jobs_send: function(id){
		$.post('/index.php?go=confirmation&act=jobs_send', {id: id}, function(d){
			...
			setInterval(function(){
				$("#user_"+id).remove();
				$("#confirm_"+id).html('<center style="color:#808080;">'+operation_completed+'</center>');
			}, 1000);
		});
	},
	jobs_del: function(id){
		$.post('/index.php?go=confirmation&act=jobs_del', {id: id}, function(d){
			if(d == '1'){
				Box.Info('msg_info', successfully, vacancy_removed, 300);
			}
			else if(d == '2'){
				Box.Info('msg_info', error, unknown_error, 300);
			}
...
var AlbumsGroups = {
        CreatAlbum: function(j){
                ...
                $.post('/index.php?go=groups_albums&act=create_page'+pid, {pid: j},  function(data){
                        Box.Show('albums', 410, lang_title_new_album, data, lang_box_canсel, create_album, 'StartCreatAlbum(); return false;', 0, 0, 0, 0);
                        $('#name').focus();
                        Page.Loading('stop');
                });
        },
        Delete: function(id, hash){
                Box.Show('del_album_'+id, 410, delete_album , '<div style="padding:15px;">'+lang_del_album+'</div>', lang_box_canсel, lang_delete, 'AlbumsGroups.StartDelete('+id+', \''+hash+'\'); return false;');
        },
        StartDelete: function(id, hash){
                var pid = '&pid='+$('#pid').val();
                $('#box_loading').show();
        ...
        MovePhoto: function(aid){
                ...
                $.get('/index.php?go=groups_albums&act=box_move_photo'+pid, {aid: aid}, function(data){
                        Box.Show('movephotos', 400, moving_photo, data, lang_msg_close);
                });
        },
...
var PhotoGroups = {
        ...
        Loading: function(f){
                if(f == 'start'){
                        ...
                        '<center><img src="/templates/Old/assets/img/loading_mini.gif" alt="" /></center>'+
                        '</div>'+
                        '</div>';
...
var Photo = {
	...
	prev_users: function(i){
		if($('#load_rate_prev_ubut').text() == lang_show_previous_estimates){
			...
			$.post('/index.php?go=photo&act=view_rating', {pid: i, lid: lid}, function(d){
				...
				$('#load_rate_prev_ubut').text(lang_show_previous_estimates);
				if(!d) $('#rate_prev_ubut').hide();
			});
		}
	},
	delrate: function(i){
		$('#delrate'+i).html(lang_score_deleted);
	...
	Loading: function(f){
		if(f == 'start'){
			...
			'<center><img class="load_photos" src="/templates/Old/assets/img/loading_mini.gif" alt="" /></center>'+
			'</div>';'</div>';
			$('body').append(loadcontent);
			$('#photo_load').show();
	...
	EditBox: function(id, r){
		...
		$.get('/index.php?go=albums&act=editphoto', {id: id}, function(data){
			Box.Show('edit_photo_'+id, '400', lang_editing_photo, '<div class="box_ppad"><div  style="color:#888;padding-bottom:5px;"><b>'+lang_albums_add_photo+'</b></div><textarea class="inpst" id="descr_'+id+'" style="width:355px;height:71px;">'+data+'</textarea></div>', lang_box_canсel, lang_box_save, 'Photo.SaveDescr('+id+', '+r+'); return false');
			Page.Loading('stop');
		});
	},	
	SaveDescr: function(id, r){
...
$('#online_friends').addClass('active_link');
		...
		if(for_id){
			...
			if(data == 'antispam_err'){
				  ...
				  return false;
					...
					Box.Info('add_demand_k_'+for_id, lang_take_ok, lang_user_added_friends, 300);
	...
	delet: function(user_id, atype){
		...
		Box.Show('del_friend_'+user_id, 410, lang_title_del_photo, '<div style="padding:15px;text-align:center"><img src="'+ava+'" alt="" /><br /><br />'+lang_fave_info+'</div>', lang_box_canсel, lang_box_yes, 'friends.goDelte('+user_id+', '+atype+'); return false');
	},
...
var fave = {
	add: function(fave_id){
		...
		$.post('/index.php?go=fave&act=add', {fave_id: fave_id}, function(data){
			...
			$('#text_add_fave').html('<i class="fa fa-star" aria-hidden="true"> '+lang_del_fave+'</span>');
			$('#addfave_load').hide();
		});
	},
	delet: function(fave_id){
		...
		$.post('/index.php?go=fave&act=delet', {fave_id: fave_id}, function(data){
			...
			$('#text_add_fave').html('<i class="fa fa-star-o" aria-hidden="true"> '+lang_add_fave+'</span>');
			$('#addfave_load').hide();
		});
	},
...
var messages = {
	new_: function(user_id){
		...
		'<div class="texta" style="width:100px">'+lang_subject+'</div><input type="text" id="theme" class="inpst" maxlength="255" style="width:300px" /><div class="mgclr"></div>'+
		'<div class="texta" style="width:100px">'+lang_message+'</div><textarea id="msg" class="inpst" style="width:300px;height:120px;"></textarea><div class="mgclr"></div>'+
		'</div>';
	...
	search: function(folder){
		...
		if(msg_query != 0 && msg_query != lang_Search_messages_received && msg_query != lang_search_sent_messages){
	...
	reply: function(for_user_id, type){
		...
		if(msg != 0 || attach_files != 0){
			...
			$.post('/index.php?go=messages&act=send', {for_user_id: for_user_id, theme: theme, msg: msg, attach_files: attach_files}, function(data){
				...
				if(type == 'reply')
					butloading('msg_sending', 50, 'enabled', lang_reply);
				else
					butloading('msg_sending', 56, 'enabled', lang_send);
			});
		} else {
			$('#msg_value').val('');
			$('#msg_value').focus();
...
var notes = {
		...
		if(title != 0){
			if(text != 0 && text != '<br>' && text != '<br />' && text != '<p>'){
				...
				$.post('/index.php?go=notes&act=save', {title: title, text: text, mywall: mywall}, function(d){
					...
					butloading('notes_sending', 74, 'enabled', lang_publish );
				});
			} else {
				iWin.focus();
		...
		if(title != 0){
			if(text != 0 && text != '<br>' && text != '<br />' && text != '<p>'){
				...
				$.post('/index.php?go=notes&act=editsave', {note_id: note_id, title: title, text: text}, function(d){
					...
					butloading('notes_sending', 111, 'enabled', lang_save_changes);
				});
			} else {
				iWin.focus();
				$("#text").css('background', '#ffefef').focus();
	...
	addcomment: function(note_id){
		...
		if(textcom != 0){
			...
			$.post('/index.php?go=notes&act=addcomment', {note_id: note_id, textcom: textcom}, function(data){
				...
				$('#textcom').val('');
				butloading('add_comm_notes', 119, 'enabled', lang_add_comment);
			});
		} else {
			$('#textcom').focus();
			$('#textcom').val('');
  ...
  delcomment: function(note_id){
    Box.Show('del_note_'+note_id, 400, lang_title_del_photo, '<div style="padding:15px;" id="text_del_quest">'+lang_del_note+'</div>', lang_box_canсel, lang_box_yes, 'notes.deletcomment('+note_id+'); return false');
  },
  
	deletcomment: function(comm_id,note_id, lnk, uid){    
...
var subscriptions = {
	add: function(for_user_id){
		...
		$.post('/index.php?go=subscriptions&act=add', {for_user_id: for_user_id}, function(d){
			...
			$('#text_add_subscription').html('<i class="fa fa-share-square" aria-hidden="true"></i> '+lang_unsubscribe+'</span>');
	...
	del: function(del_user_id){
		...
		$.post('/index.php?go=subscriptions&act=del', {del_user_id: del_user_id}, function(){
			$('#addsubscription_load').hide();
			$('#text_add_subscription').html('<i class="fa fa-share" aria-hidden="true"></i> '+lang_subscription+'</span>');
...
var videos = {
	...
	loading: function(f){
		if(f == 'start'){
			...
			'<div class="video_show_object" style="padding-top:230px;height:230px"><center><img src="/templates/Old/assets/img/loading_mini.gif" alt="" /></center></div><div class="video_show_panel" style="height:20px"></div>'+
			'</div>'+
			'</div>';
			$('body').append(loadcontent);
			$('#video_load').show();
...
var gSearch = {
	...
	go: function(){
		...
		if(query == lang_this_search || !query)
		...
		if(query == lang_this_search || query == lang_search){
			query = '';
...
var wall = {
	...
	send: function(){
		wall_text = $('#wall_text').val();
    wall_text_input = $('#vote_title').val();
    vote_answer_input = $('#vote_answer_1').val();
    vote_answer_input2 = $('#vote_answer_2').val();
		attach_files = $('#vaLattach_files').val();
    test = 'good';
		...
		if(wall_text != 0 || attach_files != 0 && wall_text_input !='' && vote_answer_input !='' && vote_answer_input2 !=''){
	...
	fast_comm_del: function(rid){
		...
		$.post('/index.php?go=wall&act=delet', {rid: rid});
    Box.Close('wall_fast_comment_'+rid);
	},
  
  
  walldelcomment: function(rid){
    Box.Show('wall_fast_comment_'+rid, 400, lang_title_del_photo, '<div style="padding:15px;" id="text_del_quest">'+lang_del_note+'</div>', lang_box_canсel, lang_box_yes, 'wall.fast_comm_del('+rid+'); return false');
  },
	...
	page: function(for_user_id){
		if($('#wall_link').text() == lang_wall_all_lnk){
			textLoad('wall_link');
	...
	attach_menu: function(type, id, show_id){
		if(type == 'open'){
			...
			$('#'+show_id).show();
			$('#wall_attach_menu').addClass(' selected');
		}
		if(type == 'close'){
			$('#'+show_id).hide();
			$('#'+id).removeClass('wall_attach_selected');
			$('#wall_attach_menu').removeClass(' selected');
		}
	},
  chatID:'',
  getUid: function (id){
    ...
    if(type == 'smile'){
      ...
      smile = data.split('smiles/');
      res_attach_id = 'smile_'+attach_id;
      $('#attach_files-'+ this.chatID).append('<span id="attach_file_'+res_attach_id+'" class="attach_file"><img src="'+data+'" class="wall_attach_smile fl_l" onClick="wall.attach_delete(\''+res_attach_id+'\', \'smile|'+smile[1]+'||\')" onMouseOver="myhtml.title(\''+res_attach_id+'\', \''+lang_wall_no_atttach+'\', \'wall_smile_\')" onMouseOut="myhtml.title_close(\''+res_attach_id+'\')" id="wall_smile_'+res_attach_id+'" style="margin-top:0px" /></span>');
      $('#vaLattach_files-'+ this.chatID).val($('#vaLattach_files-'+this.chatID).val()+'smile|'+smile[1]+'||');
      $('#attach_files-'+this.chatID).show();
    }
    ...
    if(type == 'photo'){
      ...
      res_attach_id = 'photo_'+attach_id;
      $('#attach_files-'+this.chatID).append('<span id="attach_file_'+res_attach_id+'" class="attach_file"><div class="wall_attach_photo fl_l"><div class="wall_attach_del" onMouseOver="myhtml.title(\''+res_attach_id+'\', \''+lang_wall_no_atttach+'\', \'wall_photo_\')" onMouseOut="myhtml.title_close(\''+res_attach_id+'\')" onClick="wall.attach_delete(\''+res_attach_id+'\', \'photo_u|'+action_url+'||\')" id="wall_photo_'+res_attach_id+'"></div><img src="'+data+'" alt="" /></div></span>');
      $('#vaLattach_files-'+this.chatID).val($('#vaLattach_files-'+this.chatID).val()+'photo_u|'+action_url+'||');
      $('#attach_files-'+this.chatID).show();
    }
    ...
    if(type == 'video'){
      ...
      action_url = action_url.replace('http://'+location.host+'/uploads/videos/'+aPslit[2]+'/', '');
      $('#attach_files-'+this.chatID).append('<span id="attach_file_'+res_attach_id+'" class="attach_file"><div class="wall_attach_photo fl_l"><div class="wall_attach_del" onMouseOver="myhtml.title(\''+res_attach_id+'\', \''+lang_wall_no_atttach+'\', \'wall_video_\')" onMouseOut="myhtml.title_close(\''+res_attach_id+'\')" onClick="wall.attach_delete(\''+res_attach_id+'\', \'video|'+action_url+'||\')" id="wall_video_'+res_attach_id+'"></div><img src="'+data+'" alt="" /></div></span>');
      $('#vaLattach_files-'+this.chatID).val($('#vaLattach_files-'+this.chatID).val()+'video|'+action_url+'||');
      $('#attach_files-'+this.chatID).show();
    }
    ...
    if(type == 'audio'){
      ...
      res_attach_id = 'audio_'+attach_id;
      $('#attach_files-'+this.chatID).append('<span id="attach_file_'+res_attach_id+'" class="attach_file fl_l" style="display:block;width:100%"><div class="audio_wall_attach"><div class="fl_l"><b>'+artist+'</b> &ndash; '+name+'</div><img src="/templates/Old/images/close_a.png" onMouseOver="myhtml.title(\''+res_attach_id+'\', \''+lang_wall_no_atttach+'\', \'wall_audio_\')" onClick="wall.attach_delete(\''+res_attach_id+'\', \'audio|'+action_url+'||\')" id="wall_audio_'+res_attach_id+'" class="fl_l cursor_pointer" style="margin-left:5px;margin-top:1px" /></span></div>');
      $('#vaLattach_files-'+this.chatID).val($('#vaLattach_files-'+this.chatID).val()+'audio|'+action_url+'||');
      $('#attach_files-'+this.chatID).show();
    }
	...
	attach_addDoc: function(){
		...
		Box.Page('/index.php?go=doc', '', 'all_doc', 627, select_document, lang_box_canсel, 0, 0, 400, 1, 0, 1, 0, 0);
	},
			...
			if($('#status_text').val() != 0){
				$.post('/index.php?go=groups&act=wall_send', {id: i, wall_text: $('#status_text').val()}, function(data){
					...
					if($('#rec_num').text() > 10){
						...
						$('#load_wall_all_records').html(lang_wall_all_lnk);
					}
				});
			}
		} else {
		...
		if(tell_friends){
			if($('#status_texts').val() != 0){
				$.post('/index.php?go=groups&act=wall_send', {id: i, wall_text: $('#status_texts').val()}, function(data){
					if($('#rec_num').text() == lang_no_records)
						$('.albtitle').html('<b id="rec_num">1</b>'+lang_entry);
					else
						updateNum('#rec_num', 1);
...
var news = {
	page: function(){
		...
		if($('#loading_news').text() == lang_news_prev){
			...
			$.post('/index.php?go=news&type='+type, {page: 1, page_cnt: page_cnt}, function(d){
				if(d != 'no_news'){
					...
					$('#loading_news').html(lang_news_prev);
					page_cnt++;
				} else
...
var settings = {
	addvip: function() {
		$.get('/index.php?go=vip&act=addvip', function(data){
			if(data == 'now_vip') {
				Box.Info('err', error, 'Вы уже являетесь випом!', 130, 3300);
			} else if(data == 'n_money') {
				Box.Info('err', error, 'У вас недостаточно голосов на балансе!', 222, 3300);
			} else {
...
addobshenie: function() {
	...
	savenewmail: function(){
		...
		if(settings.isValidEmailAddress(email)){
			...
			$.post('/index.php?go=settings&act=change_mail', {email: email}, function(d){
				if(d == 1){
					$('#err_email').html(mail_address_already_taken).show();
				} else {
					$('#err_email').hide();
					$('#ok_email').show();
				}
				butloading('saveNewEmail', '88', 'enabled', lang_box_save);
			});
		} else {
			$('#err_email').show();
			...
			if(new_pass != 0){
				if(new_pass2 != 0){
					...
					$.post('/index.php?go=settings&act=newpass', {old_pass: old_pass, new_pass: new_pass, new_pass2: new_pass2}, function(data){
						...
						butloading('saveNewPwd', 87, 'enabled', lang_change_password);
					});
				} else
		...
		if(name.length >= 2 && name != 0 && settings.isValidName(name)){
			if(lastname.length >= 2 && lastname != 0 && settings.isValidName(lastname)){
				...
				$.post('/index.php?go=settings&act=newname', {name: name, lastname: lastname}, function(data){
					$('.name_errors').hide();
					$('#ok_name').show();
					butloading('saveNewName', 69, 'enabled', lang_change_name);
				});
			} else {
				$('.name_errors').hide();
				$('#err_name_1').show();
	...
	addblacklist: function(bad_user_id){
		...
		$.post('/index.php?go=settings&act=addblacklist', {bad_user_id: bad_user_id}, function(){
			...
			$('#text_add_blacklist').html('<span id="text_add_blacklist"><i class="fa fa-unlock" aria-hidden="true"></i> '+lang_unlock+'</span>');
	...
	delblacklist: function(bad_user_id, type){
		...
		$.post('/index.php?go=settings&act=delblacklist', {bad_user_id: bad_user_id}, function(){
			if(type){
				...
				$('#text_add_blacklist').html('<span id="text_add_blacklist"><i class="fa fa-lock" aria-hidden="true"></i> '+lang_block+'</span>');
				$('#addblacklist_load').hide();
			} else {
	...
	conversion: function(){
		...
		$.post('/index.php?go=settings&act=conversion', function(d){
			if(d == 1) $('#ok_conversion').text(lang_recalculation_day).show();
			else $('#ok_conversion').show();
			butloading('conversion', '209', 'enabled', lang_recalculate_events);
		});
	},
	verification: function(){
		...
		if(!pravila){
			addAllErr(lang_you_agreement, 3300);
			return false;
		}
		if(skype != 0){
			if(docval != '/templates/Old/images/1.jpg' && docval2 != '/templates/Old/images/2.jpg'){
				...
				addAllErr(lang_download_document, 3300);
		} else
	...
	set_short_link: function(short_link){
		if (settings.isValidSL(short_link)) {
			...
			butloading('saveShortLink', 69, 'enabled', lang_edit_short_link);
		} else {
			$('.name_errors').hide();
			$('#err_sl_1').show();
			setErrorInputMsg('short_link');
...
var support = {
	send: function(){
		...
		if(title != 0 && title != add_headline_question){
			if(question != 0 && question != tell_about_problem_little){
				$('#cancel').hide();
	...
	answer: function(qid, uid){
		...
		if(answer != 0 && answer != lang_comment){
			butloading('send', '56', 'disabled', '');
			$.post('/index.php?go=support&act=answer', {answer: answer, qid: qid}, function(data){
				if(uid == 0)
					$('#status').text(there_answer);
				else
					$('#status').text(question_awaits_processing);
				$('#answers').append(data);
				$('#answer').val('');
				butloading('send', '56', 'enabled', lang_box_send);
			});
...
var groups = {
	sendver: function(i){
		...
		$.post('/index.php?go=groups&act=sendver', {public_id: i}, function(d){
			Page.Loading('stop');
			if(d == 1) addAllErr(lang_in_verify_page, 5000);
			else {
				$('#sendver').html(lang_demand_ok);
				$('#sendverlnk').attr('onClick', 'return false');
			}
		});
	},
	createbox: function(){
		Box.Show('create', 490, lang_groups_new, '<div style=""><div class="videos_text">'+lang_name+'</div><input type="text" class="videos_input" id="title" maxlength="65" /></div>', lang_box_canсel, lang_groups_cretate, 'groups.creat()', 100, 0, 0, 0, 0, 'title');
		$('#title').focus();
	},
	creat: function(){
		var title = $('#title').val();
	...
	exit: function(id){
		$('#exitlink'+id).html('<div class="color77777" style="margin-top:6px;margin-right:7px">'+lang_you_company+'</div>');
	...
	exit2: function(id, user_id){
		...
		if($('#traf').text() == 0){
			$('#users_block').hide();
			$('#num2').html('<span class="color777">'+lang_you_will_first+'</span>');
		}
	...
	addcontact: function(id){
		Box.Page('/index.php?go=groups&act=addfeedback_pg', 'id='+id, 'addfeedback', 400, lang_adding_contact_person, lang_box_canсel, lang_box_save, 'groups.savefeedback('+id+')', 0, 0, 0, 0, 'upage', 0);
	},
	savefeedback: function(id){
		...
		if($('#feedimg').attr('src') != template_dir+'/images/contact_info.png'){
			...
			$.post('/index.php?go=groups&act=addfeedback_db', {id: id, upage: upage, office: office, phone: phone, email: email}, function(d){
				if(d == 1){
					Box.Info('err', lang_dd2f_no, lang_user_already_contact_list, 300, 2000);
	...
	allfeedbacklist: function(id){
		Box.Page('/index.php?go=groups&act=allfeedbacklist', 'id='+id, 'allfeedbacklist', 450, lang_contacts, lang_msg_close, 0, 0, 300, 1, 1, 1, 0, 0);
	},
	delfeedback: function(id, uid){
		...
		updateNum('#fnumu');
		if(si <= 0){
			$('#feddbackusers').html('<div class="line_height color777" align="center">'+lang_representa_pages_phone+'<br /><a href="/public'+id+'" onClick="groups.addcontact('+id+'); return false">'+lang_add_contacts+'</a></div>');
			$('.box_conetnt').html('<div align="center" style="padding-top:10px;color:#777;font-size:13px;">'+lang_contact_list_empty+'</div><style>#box_bottom_left_text{padding-top:6px}</style>');
		}
	...
	saveinfo: function(id){
		...
		if(pattern.test(adres_page)){
			...
			$.post('/index.php?go=groups&act=saveinfo', {id: id, title: title, descr: descr, comments: comments, adres_page: adres_page, discussion: $('#discussion').val(), background_repeat: $('#background_repeat').val(), web: web}, function(d){
				if(d == 'err_adres')
					Box.Info('err', error, lang_address_already_taken, 130, 1500);
				else
				...
				butloading('pubInfoSave', 55, 'enabled', lang_box_save);
			});
		} else {
			setErrorInputMsg('adres_page');
			Box.Info('err', error, lang_consisting_letters_numbers+' «_» .', 300, 5500);
		}
	},
	editform: function(){
	...
	addadmin: function(id){
		...
		if(new_admin_id && !check_adm){
			Box.Page('/index.php?go=groups&act=new_admin', 'new_admin_id='+new_admin_id, 'new_admin_id', 400, lang_appointment_manager, lang_msg_close, lang_assign_leader, 'groups.send_new_admin('+id+', '+new_admin_id+')', 130, 0, 0, 0, 0, 0);
		} else
			addAllErr(lang_user_already_managers);
	},
	send_new_admin: function(id, new_admin_id){
		var ava = $('#adm_ava').attr('src');
		var adm_name = $('#adm_name').text();
		var data = '<div class="public_oneadmin" id="admin'+new_admin_id+'"><a href="/u'+new_admin_id+'" onClick="Page.Go(this.href); return false"><img src="'+ava+'" align="left" width="32" /></a><a href="/u'+new_admin_id+'" onClick="Page.Go(this.href); return false">'+adm_name+'</a><br /><a href="/" onClick="groups.deladmin(\''+id+'\', \''+new_admin_id+'\'); return false"><small>'+lang_delete+'</small></a></div>';
  ...
  delwallcomment: function(rec_id){
    Box.Show('wall_fast_comment_'+rec_id, 400, lang_title_del_photo, '<div style="padding:15px;" id="text_del_quest">'+lang_del_note+'</div>', lang_box_canсel, lang_box_yes, 'groups.comm_wall_delet('+rec_id+'); return false');
  },
	wall_delet: function(rec_id){
		$('#wall_record_'+rec_id).html('<div style="margin-bottom:15px"><span class="color77777 bb_delete">'+lang_entry_deleted+'</span></div>');
		$('#wall_fast_block_'+rec_id+', .wall_fast_opened_form').remove();
		$('#wall_record_'+rec_id).css('padding-bottom', '5px');
		myhtml.title_close(rec_id);
		updateNum('#rec_num');
		$.post('/index.php?go=groups&act=delwallcomment', {rec_id: rec_id});
	},
	comm_wall_delet: function(rec_id, public_id){
		$('#wall_fast_comment_'+rec_id).html('<div class="color77777 bb_delete">'+comment_has_deleted+'</div>');
		$.post('/index.php?go=groups&act=delwallcomment', {rec_id: rec_id, public_id: public_id});
    Box.Close('wall_fast_comment_'+rec_id);	
	},
	...
	wall_page: function(){
		...
		if($('#load_wall_all_records').text() == lang_wall_all_lnk && $('#rec_num').text() > 10){
...
'<div class="photo_close" onClick="Photo.Close(\'\'); return false;"></div>'+
 '<div class="photo_bg" style="min-height:400px;   background: none repeat scroll 0 0 rgba(0, 0, 0, 0); box-shadow: none;">'+
  ...
  '<div class="photo_img_box cursor_pointer main_img_box" onClick="Photo.Close(\'\'); return false"><img src="'+photo+'"  id=\"photo_view_src\" style="margin-bottom:7px; box-shadow: 0 2px 33px -2px rgba(0, 0, 0, 0.8);" /></div><div class="line_height">'+
...
'</div>';
	...
	wall_video_add_box: function(){
		wall.attach_menu('close', 'wall_attach', 'wall_attach_menu');
		Box.Show('attach_videos', 400, link_video_mixnet, '<div style="padding:15px;"><input  type="text"  placeholder="'+enter_video_link_mixnet+'"  class="videos_input" id="video_attach_lnk" style="width:355px;margin-top:10px" /></div>', lang_box_canсel, lang_attach, 'groups.wall_video_add_select()');
	...
	all_people: function(public_id, page_num){
		...
		Box.Page('/index.php?go=groups&act=all_people', 'public_id='+public_id+'&num='+num+page, 'all_peoples_users_'+public_id+page_num, 525, lang_subscribers, lang_msg_close, 0, 0, 345, 1, 1, 1, 0, 1);
	},
	all_groups_user: function(for_user_id, page_num){
		...
		Box.Page('/index.php?go=groups&act=all_groups_user', 'for_user_id='+for_user_id+'&num='+num+page, 'all_groups_users_'+for_user_id+page_num, 525, interesting_pages, lang_msg_close, 0, 0, 345, 1, 1, 1, 0, 1);
	},
	...
	inviteSet: function(i){
	  ...
	  if(!check){
		if(numCheck >= limit){
		  Box.Info('load_photo_er', lang_dd2f_no, invite_company_than && limit && friends_once, 380, 3000);
		  return false;
	...
	inviteSend: function(i){
	  ...
	  $.post('/index.php?go=groups&act=invitesend', {id: i, ulist: userInviteList}, function(d){
	    if(d == 1) Box.Info('load_photo_er', lang_dd2f_no, lang_dd2f_no, invite_company_than && 50 && friends_once, 380, 3000);
		else Box.Info('load_photo_er', lang_dd2f_no, lnvitations_successfully_sent, 230, 2600);
		viiBox.clos('inviteBox', 1);
	  });
	},
	inviteFriendsPage: function(i){
      if($('#load_invite_prev_ubut').text() == show_more_friends){
	    ...
	    $.post('/index.php?go=groups&act=invitebox', {page_cnt: page_cnt_invite, id: i}, function(d){
		  ...
		  $('#load_invite_prev_ubut').text(show_more_friends);
		  if(!d) $('#invite_prev_ubut').remove();
	    });
	  }
	},
	InviteOk: function(i){
	  $('#action_'+i).html('<span class="color77777">'+joined_company+'</span>');
	  $.post('/index.php?go=groups&act=login', {id: i});
	},
	InviteNo: function(i){
	  $('#action_'+i).html('<span class="color77777">'+invitation_denied+'</span>');
	  $.post('/index.php?go=groups&act=invite_no', {id: i});
	},
	invitePage: function(){
      if($('#load_gr_invite_prev_ubut').text() == show_more_invites){
	    ...
	    $.post('/index.php?go=groups&act=invites', {page_cnt: page_cnt_invite_gr}, function(d){
		  ...
		  $('#load_gr_invite_prev_ubut').text(show_more_invites);
...
var audio = {
    ...
    $('.js_titleRemove').remove(); 
    Box.Close(); 
    Box.Show('addaudio', 510, lang_audio_add, '<div class="videos_pad"><div class="buttonsprofile albumsbuttonsprofile buttonsprofileSecond" style="height:22px;margin-bottom:20px;margin-top:-5px"><div class="buttonsprofileSec cursor_pointer"><a><div><b>'+$('#cl').val()+'</b></div></a></div><a class="cursor_pointer" onClick="audio.addBoxComp()"><div><b>'+$('#upload-comp').val()+'</b></div></a></div><div class="videos_text">'+$('#insert-link').val()+'</div><input type="text" class="videos_input" id="audio_lnk" style="margin-top:5px" /><span id="vi_info">'+$("#for-examples").val()+'<b>http://music.com/uploads/files/audio/2012/faxo_-_kalp..</b></span></div>', lang_box_canсel, lang_album_create, 'audio.send()', 0, 0, 1, 1); 
    $('#audio_lnk').focus(); 
  },  
	addBoxComp: function(){
		Box.Close();
		Box.Show('addaudio_comp', 510, lang_audio_add, '<div class="videos_pad"><div class="buttonsprofile albumsbuttonsprofile buttonsprofileSecond" style="height:22px;margin-bottom:20px;margin-top:-5px"><a onClick="audio.addBox()" class="cursor_pointer"><div><b>'+$('#cl').val()+'</b></div></a><div class="buttonsprofileSec cursor_pointer"><a><div><b>'+$('#upload-comp').val()+'</b></div></a></div></div><div class="videos_text">'+$('#limit').val()+'<div class="clear"></div><li style="font-weight:normal;color:#000;font-size:11px;margin-top:10px">'+$('#audio-file').val()+' MP3.</li><li style="font-weight:normal;color:#000;font-size:11px;margin-bottom:15px">'+$('#audio-file-violet').val()+'</li><div class="button_div fl_l" style="margin-left:170px"><button id="upload">'+$('#audio-file-upload').val()+'</button></div><div class="clear"></div><div style="margin-top:15px;font-size:11px;color:#000;font-weight:normal">'+$('#file-upload-desc').val()+' <a href="/?go=search&type=5"><b>'+$('#search-music').val()+'.</b></a></div></div></div>', lang_box_canсel, lang_album_create, 'audio.send()', 0, 0, 1, 1);
		$('#audio_lnk').focus();
		$('#box_but').hide();
	...
	edit: function(aid, pid){
		...
		Box.Show('edit'+aid, 510, ' ' + $('#edit-audios').val()+ ' ', '<div class="videos_pad"><div class="videos_text">'+$('#artists').val()+'</div><input type="text" class="videos_input" id="valartis'+aid+'" style="margin-bottom:15px" value="'+$('#artis'+aid).html()+'" /><div class="videos_text">'+$('#edit-names').val()+'</div><input type="text" class="videos_input" id="vaname'+aid+'" value="'+$('#name'+aid).html()+'" /></div>', lang_box_canсel, lang_box_save, funcsave, 0, 0, 1, 1);
		$('#audio_lnk').focus();
	},
...
var im = {
	typograf: function(){
		...
		var a = $('#msg_text').val() || $('.im_chatforntext').val();
		if(vii_typograf){
	...
	open: function(uid){
		...
		$.post('/index.php?go=im&act=history', {for_user_id: uid}, function(d){
			...
			$('.im_typograf').html('<div class="no_display" id="im_typograf"><img src="'+template_dir+'/images/typing.gif" /> '+aco[0]+' íàáèðàåò ñîîáùåíèå..</div>');
		...
		if(msg_text != 0 && $('#status_sending').val() == 1 || attach_files != 0){
			...
			$.post('/index.php?go=im&act=send', {for_user_id: for_user_id, my_name: my_name, my_ava: my_ava, msg: msg_text, attach_files: attach_files}, function(data){
				if(data == 'err_privacy')
					...
					butloading('sending', 56, 'enabled', $('#sendd').val());
				}
			});
		} else
	...
	page: function(for_user_id){
		...
		$('#wall_all_records').attr('onClick', '');
		if($('#load_wall_all_records').text() == 'Ïîêàçàòü ïðåäûäóùèå ñîîáùåíèÿ'){
			textLoad('load_wall_all_records');
			$.post('/index.php?go=im&act=history', {first_id: first_id, for_user_id: for_user_id}, function(data){
				...
				if(!data){
					...
					$('#load_wall_all_records').html('Ïîêàçàòü ïðåäûäóùèå ñîîáùåíèÿ');
				}
			});
		}
	},
	box_del: function(u){
		Box.Show('im_del'+u, 350, 'Óäàëèòü âñå ñîîáùåíèÿ', '<div style="padding:15px;" id="del_status_text_im">Âû äåéñòâèòåëüíî õîòèòå óäàëèòü âñþ ïåðåïèñêó ñ äàííûì ïîëüçîâàòåëåì?<br /><br />Îòìåíèòü ýòî äåéñòâèå áóäåò íåâîçìîæíî.</div>', lang_box_cancel, lang_box_yes, 'im.del('+u+')');
	},
	del: function(u){
		$('#box_loading').show();
		ge('box_butt_create').disabled = true;
		$('#del_status_text_im').text('Ïåðåïèñêà óäàëÿåòñÿ..');
		$.post('/index.php?go=im&act=del', {im_user_id: u}, function(d){
			Box.Close('im_del'+u);
			Box.Info('ok_im', 'Èñòîðèÿ ïåðåïèñêè óäàëåíà', 'Âñå ñîîáùåíèÿ äèàëîãà áûëè óñïåøíî óäàëåíû.', 300, 3000);
			$('#okim'+u).remove();
		});
	},
...
var Report = {
	Box: function(act, id){
		Box.Close();
		if(act == 'photo') lang_report = complaint_photo;
		else if(act == 'video') lang_report = video_complaint;
		else if(act == 'note') lang_report = comment_complaint;
		else lang_report = '';
		Box.Show('report', 400, lang_report, '<div class="report_pad">'+please_choose_reason+'<div class="clear"></div><br /><select id="type_report" class="form-control inpst" style="width:212px" onChange="if(this.value > 1) {$(\'#report_comm_block\').show();$(\'#text_report\').focus()} else {$(\'#report_comm_block\').hide();$(\'#text_report\').val(\'\')}"><option value="1">'+material_adults+'</opyion><option value="2">'+child_pornography+'</opyion><option value="3">'+extremism+'</opyion><option value="4">'+violence+'</opyion><option value="5">'+propaganda_drugs+'</opyion></select><div class="clear"></div><div id="report_comm_block" class="no_display"><br />'+comments+':<br /><br /><textarea id="text_report" class="inpst form-control" style="width:200px;height:80px"></textarea></div></div>', lang_msg_close, lang_box_send, 'Report.Send(\''+act+'\', '+id+')');
		$('#audio_lnk').focus();
		$('#video_object').hide();
	},
	Send: function(act, id){
		...
		$.post('/index.php?go=report', {act: act, id: id, type_report: $('#type_report').val(), text_report: $('#text_report').val()}, function(d){
			Box.Close();
			Box.Info('yes_report', thank_you, complaint_has_been_sent, 300, 3000);
			$('#video_object').show();
		});
	},
	WallSend: function(act, id){
		$('#wall_record_'+id).html('<div class="color7737">'+message_marked_as_spam+'</div>');
...
var Repost = {
	Box: function(rec_id, g_tell){
		Box.Page('/index.php?go=repost&act=all', 'rec_id='+rec_id, 'repost', 430, sending_records, lang_box_canсel, share_this_entry, 'Repost.Send('+rec_id+', '+g_tell+')', 0, 0, 0, 0, 'comment_repost');
	},
	Send: function(rec_id, g_tell){
		...
		$.post('/index.php?go=repost&act='+cas, {rec_id: rec_id, comm: comm, sel_group: $('#sel_group').val(), g_tell: g_tell, for_user_id: $('#for_user_id').val()}, function(d){
			if(d == 1){
				...
				if(type == 1) Box.Info('yes_report', record_has_sent, entry_will_appear_news_friends, 300, 2500);
				if(type == 2) Box.Info('yes_report', record_has_sent, entry_company_page, 300, 2500);
				if(type == 3) Box.Info('yes_report', lang_msg_ok_title, lang_msg_ok_text, 300, 2500);
				Box.Close();
			}
		});
	}
...
var Doc = {
	...
	Del: function(did){
		$('.js_titleRemove').remove();
		$('#doc_block'+did).html(document_was_deleted);
		updateNum('#upDocNum');
		langNumric('langNumric', $('#upDocNum').text(), the_document, the_documenta, the_documents, the_document, the_documents);
...
var Votes = {
	AddInp: function(){
		...
		if($('#answerNum').val() == 10) $('#addNewAnswer').html(lang_add);
		if($('#answerNum').val() > 2) $('#addDelAnswer').html('<a class="cursor_pointer" onClick="Votes.DelInp()">'+lang_delete+'</a>');
	...
	DelInp: function(id){
		if($('#answerNum').val() > 2){
			...
			$('#addNewAnswer').html('<a class="cursor_pointer" onClick="Votes.AddInp()">'+lang_add+'</a>');
		}
		if($('#answerNum').val() == 2) $('#addDelAnswer').html(lang_delete);
	},
...
var attach = {
	...
	delet_comm: function(i, p){
		$('#comment_'+i).html('<div class="color777" style="margin-bottom:5px">'+comment_has_deleted+'</div>');
		$.post('/index.php?go=attach_comm&act=delcomm', {id: i, purl: p});
	},
	page: function(p){
		if($('#load_attach_comm_msg_lnk').text() == previous_comments){
			...
			$.post('/index.php?go=attach_comm&act=prevcomm', {purl: p, first_id: $('.attach_comm_photo:first').attr('id').replace('comment_', ''), page: page}, function(d){
				...
				$('#load_attach_comm_msg_lnk').text(previous_comments);
				if(!d){
					$('#load_attach_comm_msg_lnk').text(lang_wall_hide_comm).css('background', '#fff');
	...
	hide_page: function(f){
		...
		$('#load_attach_comm_msg_lnk').text(previous_comments).css('background', 'rgb(233, 237, 241)');
		$('#attachcommPrev').html('');
		page = 1;
	},
}
...
$('#cover_restart_profile').html('<div style="width:100%;height:'+hi+'px;position:relative;top:0px;z-index:1" id="les10_ex2_profile"><img src="'+i+'" width="600" id="cover_img" /></div>');
$('.cover_addut_profile, .cover_descring_profile').show();
$('.cover_newposswrt_profile').css('background', 'url("") repeat scroll 0 0 rgba(0, 0, 0, 0.59)');
coverpro.init(i, hi);
}
...
if(d <= h){
$("#les10_ex2_profile").remove();
$('#cover_restart_profile').html('<div style="width:100%;height:'+hi+'px;position:relative;top:'+h+'px;z-index:1" id="les10_ex2_profile"><img src="'+i+'" width="600" id="cover_img" /></div>');
$('.cover_addut_profile, .cover_descring_profile').show();
$('.cover_newposswrt_profile').css('background', 'url("") repeat scroll 0 0 rgba(0, 0, 0, 0.59)');
coverpro.init(i, hi);
}
...
$('#upload_cover_profile').show().text(add_cover_photo);
...
$('.cover_newpos_profile').css('width', '100%');
$('.tabs').show();
$('.cover_newposswrt_profile').css('background', 'none');
$('#cover_img').attr('src', '');
...
var coverg = {
	init: function(i, hi){
		...
		$("#les10_ex2").draggable({
			...
			drag: function(event, ui){
				...
				if(d >= 0){
					$("#les10_ex2").remove();
					$('#cover_restart').html('<div style=" width: 100%;height:'+hi+'px;position:relative;top:0px;bottom:0;z-index:1" id="les10_ex2"><img src="'+i+'" width="" id="cover_img" /></div>');
					$('.cover_addut, .cover_descring').show();
				...
				if(d <= h){
					$("#les10_ex2").remove();
					$('#cover_restart').html('<div style=" width: 100%;height:'+hi+'px;position:relative;bottom:0;top:'+h+'px;z-index:1" id="les10_ex2"><img src="'+i+'" width="" id="cover_img" /></div>');
					$('.cover_addut, .cover_descring').show();
							$('.cover_newpos').css('background', 'url("") repeat scroll 0 0 rgba(0, 0, 0, 0.7)');
					cover.init(i, hi);
				}
	...
	del: function(public_id){
		...
		$('#upload_cover').show().text(add_cover_photo);
	...
	cancel: function(t){
		...
		$('.cover_newpos').css('margin-left', '0');
	...
	startedit: function(i, h, public_id){
		$('#upload_cover').show().text(lang_fon_change);
		$('.cover_newpos').css('background', 'url("") repeat scroll 0 0 rgba(0, 0, 0, 0.7)');
...
if($('.profile_onefriend_happy').size() > 4) $('#happyAllLnk').show();
   btnUpload = $('#upload_cover_profile');
   ...
   $('#upload_cover_profile').text(lang_fon_change);
   $('.cover_profile_bg_profile').css('opacity', '1');
   $('.cover_loaddef_bg_profile').css('cursor', 'move');
   $('#upload_cover_profile').css('margin-left', '0px');
   $('#upload_cover_profile').css('margin-top', '12px');
   $('.cover_newposswrt_profile').css('width', '100%');
   $('.cover_newpos_profile').css('margin-left', '0px');
   $('.cover_newpos_profile').css('width', '100%');
   $('.cover_newposswrt_profile').css('width', '100%');
   $('.cover_newposswrt_profile').css('background', 'url("") repeat scroll 0 0 rgba(0, 0, 0, 0.59)');
   $('.tabs').hide();
...
var ava2 = {
  box: function(){
	...
	$.post('/index.php?go=editprofile&act=ava2', function(d){
	  if(d == 'no_ava'){
		Box.Info('infoava2', error, avatar_upload_main_photo, 300, 3000);
		viiBox.stop();
	  } else
		viiBox.win('ava2', d);
	});
